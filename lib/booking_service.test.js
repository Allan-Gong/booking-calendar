const { toBooking, addNewBookings } = require("./booking_service");
const {
  booking1,
  booking2,
  newBooking1,
...
test("addNewBookings - existing conflicts & conflicts found in new bookings", () => {
  // Prepare fixtures
  const existingBookings = [booking1, booking2];
  const newBookings = [
    newBooking1,
  ...
  const result = addNewBookings(
    existingBookings,
    [conflictBooking1, conflictBooking2],
    newBookings
  );
